...
function systemPrompt(origin?: { lat: number; lon: number }, today = new Date()): string {
  return [
    ...
    '  It knows streets, suburbs and addresses — NOT points of interest. A station,',
    '  stop or venue name belongs in search_stops. If it returns confident:false, the',
    '  words matched but the place may not: check the suburb in displayName first.',
