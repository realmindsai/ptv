...
## Task 5: `--hill-weight` scoring (signed bias)

**Files:**
- Modify: `src/plan/types.ts` (constants + PlanRequest)
- Modify: `src/plan/score.ts` (cost function)
- Modify: `src/commands/plan.ts` (--hill-weight flag)
- Test: `tests/unit/plan/score.test.ts`

The `--hill-weight` flag is a signed scalar that lets users push the recommendation toward flat (negative weight) OR toward hills (positive weight). Replaces the originally-proposed boolean `--prefer-flat`. Default is 0 (neutral, no behavior change).

- [ ] **Step 1: Add constants and `hillWeight` to `src/plan/types.ts`**

Add at the bottom:

```ts
export const HILL_ASCEND_WEIGHT = 0.05;     // per meter of ascend
export const HILL_SUSTAINED_WEIGHT = 0.02;  // per (grade% × meter) of sustained climb
export const HILL_FLAT_OFFSET = 0.3;        // per percent of flat fraction (subtract)
```

Add `hillWeight: number;` to `PlanRequest`:
...
Update `makeReq` to include `hillWeight: 0`:
...
`it1()` helper already allows `Partial<Itinerary>` so new fields work automatically.

Add tests inside `describe('labelAndSort()', ...)`:

```ts
  it('--hill-weight 0 (default) does not change recommendation', () => {
    // a is faster (50 min), b is hillier. With hillWeight=0, fastest wins.
    const a = it1({ totalTimeMin: 50, bikeKm: 10, ascendM: 30, flatFraction: 0.9 });
    const b = it1({ totalTimeMin: 60, bikeKm: 10, ascendM: 300, flatFraction: 0.2 });
    const out = labelAndSort([a, b], makeReq());
    expect(out.find((i) => i.labels.includes('recommended'))?.totalTimeMin).toBe(50);
  });

  it('--hill-weight -1 prefers flatter itinerary (flat bias)', () => {
    // a: 50min, 300m climb, flatFraction 0.3
    //   hilliness(a) = 0.05*300 + 0.02*0 - 0.3*30 = 15 + 0 - 9 = 6
    //   cost(a) = 50 - (-1)*6 = 50 + 6 = 56
    // b: 55min, 30m climb, flatFraction 0.9
    //   hilliness(b) = 0.05*30 + 0 - 0.3*90 = 1.5 - 27 = -25.5
    //   cost(b) = 55 - (-1)*(-25.5) = 55 - 25.5 = 29.5
    // b wins (flatter)
    const a = it1({ totalTimeMin: 50, bikeKm: 10, ascendM: 300, flatFraction: 0.3,
                    maxSustainedGradePercent: 0, maxSustainedGradeM: 0 });
    const b = it1({ totalTimeMin: 55, bikeKm: 10, ascendM: 30, flatFraction: 0.9,
                    maxSustainedGradePercent: 0, maxSustainedGradeM: 0 });
    const out = labelAndSort([a, b], makeReq({ hillWeight: -1 }));
    expect(out.find((i) => i.labels.includes('recommended'))?.ascendM).toBe(30);
  });

  it('--hill-weight +1 prefers hillier itinerary (hill bias)', () => {
    // Same a/b as previous test
    // cost(a) = 50 - (+1)*6 = 50 - 6 = 44
    // cost(b) = 55 - (+1)*(-25.5) = 55 + 25.5 = 80.5
    // a wins (hillier)
    const a = it1({ totalTimeMin: 50, bikeKm: 10, ascendM: 300, flatFraction: 0.3,
                    maxSustainedGradePercent: 0, maxSustainedGradeM: 0 });
    const b = it1({ totalTimeMin: 55, bikeKm: 10, ascendM: 30, flatFraction: 0.9,
                    maxSustainedGradePercent: 0, maxSustainedGradeM: 0 });
    const out = labelAndSort([a, b], makeReq({ hillWeight: 1 }));
    expect(out.find((i) => i.labels.includes('recommended'))?.ascendM).toBe(300);
  });

  it('--hill-weight composes with --prefer-bike-path', () => {
    // a: 50min, 5km on path, 300m climb, flatFraction 0.3
    //   hilliness(a) = 0.05*300 - 0.3*30 = 15 - 9 = 6
    //   cost(a) = 50 - 5*5 - (-1)*6 = 50 - 25 + 6 = 31
    // b: 70min, 10km on path, 30m climb, flatFraction 0.9
    //   hilliness(b) = 0.05*30 - 0.3*90 = 1.5 - 27 = -25.5
    //   cost(b) = 70 - 5*10 - (-1)*(-25.5) = 70 - 50 - 25.5 = -5.5
    // b wins
  ...
  it('--hill-weight falls back gracefully when no elevation data', () => {
    const a = it1({ totalTimeMin: 50, bikeKm: 10 });
    const b = it1({ totalTimeMin: 60, bikeKm: 10 });
    const out = labelAndSort([a, b], makeReq({ hillWeight: -2 }));
    // No elevation data → hilliness is 0 for both → cost reduces to totalTimeMin
    expect(out.find((i) => i.labels.includes('recommended'))?.totalTimeMin).toBe(50);
  });
```
...
```bash
npx vitest run tests/unit/plan/score.test.ts -t "hill-weight"
```
Expected: 4 tests fail (the `hill-weight 0` test should already pass since the default behavior is unchanged) — `hillWeight` field not yet on PlanRequest, cost function not yet extended.
...
import {
  PATH_BONUS_PER_KM,
  HILL_ASCEND_WEIGHT,
  HILL_SUSTAINED_WEIGHT,
  HILL_FLAT_OFFSET,
} from './types';
```

Find the recommended calculation block (around lines 67-76 of the v1.4 file; may differ after Task 4):
...
```ts
  ...
  function cost(i: Itinerary): number {
    ...
    if (req.hillWeight !== 0) {
      c -= req.hillWeight * hilliness(i);
    }
    return c;
  }
  const recommended = (req.preferBikePath || req.hillWeight !== 0)
    ? deduped.reduce((m, i) => (cost(i) < cost(m) ? i : m))
    : deduped.reduce((m, i) => (i.totalTimeMin < m.totalTimeMin ? i : m));
```

- [ ] **Step 5: Wire `--hill-weight` through `src/commands/plan.ts`**

Add the option after `--min-on-path-fraction`:

```ts
    .option('--hill-weight <n>', 'Bias recommended toward flat (<0) or hills (>0); 0 is neutral', parseFloat, 0)
```
...
Add `hillWeight: opts.hillWeight,` to the `req` object.

Update all inline PlanRequest literals in test files to include `hillWeight: 0,`. Search `tests/unit/plan/orchestrator.test.ts`, `tests/integration/plan.test.ts`, `tests/e2e/plan.test.ts` for `preferBikePath: false` (and `preferBikePath: false,` with trailing comma) and add `hillWeight: 0,` after each occurrence.
...
npm run test:unit
```
Expected: 84 unit tests pass (79 after Task 4 + 5 new). The hill-weight=0 test passes without code change.

```bash
npm test
```
Expected: full suite passes — ~97 tests (~84 unit + 13 integration + 12 e2e).
...
git add src/plan/types.ts src/plan/score.ts src/commands/plan.ts \
        ...
        tests/integration/plan.test.ts tests/e2e/plan.test.ts
git commit -m "feat(plan): add --hill-weight signed scoring for elevation bias"
```
...
- `--hill-weight 0` (default) leaves recommendation unchanged from previous tasks.
- Negative values penalize hills (prefer flat); positive values reward hills (prefer hills).
- The `hilliness()` function returns a single scalar combining ascend, sustained climb intensity, and flat-fraction offset. Multiplied by `hillWeight` and subtracted from cost.
- Composes additively with `--prefer-bike-path`.
- Commit to `main`. No `--no-verify`.
